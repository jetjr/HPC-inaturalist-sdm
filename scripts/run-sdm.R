...
#Assign month to observation
iNaturalist.data$month <- substr(iNaturalist.data$datetime, 6, 7)

#Get and assign most common species name
n_occur <- data.frame(table(iNaturalist.data$species_guess))
n_occur <- n_occur[order(-n_occur$Freq),]
iNaturalist.data$species <- n_occur$Var1[1]

obs.data <- iNaturalist.data[, c("scientific_name","month", "species", "longitude", "latitude")]
colnames(obs.data) <- c("name", "month", "species", "lon", "lat")
...
presence.values <- extract(x = bioclim.data, y = obs.data[4:5])
...
bc <- bioclim(x = bioclim.data, p = presence.train[4:5])
bc.eval <- evaluate(presence.test[4:5], bg.test, bc, bioclim.data)
...
plot(wrld_simpl, 
     add = TRUE,
     border = "dark grey")
title(main = paste(obs.data$name[1], ":", obs.data$species[1]))
box()
...
for (i in unique(obs.data$month)) {
  ...
  # Subset obs.data by month
  obs.data <- obs.data[which(obs.data$month == i),]
  ...
  # Save image to file
  data(wrld_simpl) # Need this for the map
  png(filename = paste0(outpath, outprefix, "-", i, "-prediction.png"))
  par(mar = c(3, 3, 3, 1) + 0.1)
  plot(wrld_simpl, 
       xlim = c(min.lon, max.lon), 
       ylim = c(min.lat, max.lat), 
  ...
  plot(wrld_simpl, 
       add = TRUE,
       border = "dark grey")
  title(main = paste(obs.data$name[1], ":", obs.data$species[1]), sub = paste("Month:", i))
  box()
  par(mar = c(5, 4, 4, 2) + 0.1)
  dev.off()
